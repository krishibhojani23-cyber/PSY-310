...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\stroop\\Memory Experiment_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_3 = visual.TextStim(win=win, name='text_3',
        text='Get ready for the TEST.\n\nIf the word was present in the list press Y\nIf the word was not present in the list press N\n\nPress SPACE to continue.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    trialsStudy = data.TrialHandler2(
        name='trialsStudy',
        nReps=1.0, 
        method='random', 
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('data/SOLO PROJECT/Memory study.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrialsStudy in trialsStudy:
        trialsStudy.status = STARTED
        if hasattr(thisTrialsStudy, 'status'):
            thisTrialsStudy.status = STARTED
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            if hasattr(thisTrialsStudy, 'status') and thisTrialsStudy.status == STOPPING:
                continueRoutine = False
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            if textstudy.status == STARTED:
                ...
                if tThisFlipGlobal > textstudy.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    textstudy.tStop = t  # not accounting for scr refresh
                    textstudy.tStopRefresh = tThisFlipGlobal  # on global time
        ...
        # mark thisTrialsStudy as finished
        if hasattr(thisTrialsStudy, 'status'):
            thisTrialsStudy.status = FINISHED
        # if awaiting a pause, pause now
        if trialsStudy.status == PAUSED:
    ...
    trialsTest = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('data/SOLO PROJECT/Memory test.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrialsTest in trialsTest:
        ...
        TestTrial = data.Routine(
            name='TestTrial',
            components=[texttest, key_resp_3],
        )
        ...
        while continueRoutine:
            ...
            if key_resp_3.status == STARTED and not waitOnFlip:
                ...
                if len(_key_resp_3_allKeys):
                    ...
                    # was this correct?
                    if (key_resp_3.keys == str(yn_keys)) or (key_resp_3.keys == yn_keys):
                        key_resp_3.corr = 1
                    else:
                        key_resp_3.corr = 0
                    # a response ends the routine
        ...
        if key_resp_3.keys in ['', [], None]:  # No response was made
            ...
            if str(yn_keys).lower() == 'none':
               key_resp_3.corr = 1;  # correct non-response
            else:
               key_resp_3.corr = 0;  # failed to respond (incorrectly)
        # store data for trialsTest (TrialHandler)
        trialsTest.addData('key_resp_3.keys',key_resp_3.keys)
        trialsTest.addData('key_resp_3.corr', key_resp_3.corr)
        if key_resp_3.keys != None:  # we had a response
            trialsTest.addData('key_resp_3.rt', key_resp_3.rt)
            trialsTest.addData('key_resp_3.duration', key_resp_3.duration)
