...
expName = 'Visual Search Experiment'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\stroop\\Visual Search Experiment.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    trial = data.Routine(
        name='trial',
        components=[fixation, target, distr],
    )
    ...
    while continueRoutine:
        ...
        # if distr is stopping this frame...
        if distr.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > distr.tStartRefresh + 1-frameTolerance:
                # keep track of stop time/frame for later
                distr.tStop = t  # not accounting for scr refresh
                distr.tStopRefresh = tThisFlipGlobal  # on global time
                distr.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'distr.stopped')
                # update status
                distr.status = FINISHED
